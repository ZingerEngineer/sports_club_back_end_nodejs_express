...
export class Init1720640294069 implements MigrationInterface {
    name = 'Init1720640294069'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "tournament" ("tournament_id" int NOT NULL IDENTITY(1,1), "tournament_name" nvarchar(120) NOT NULL, "date_held" datetime NOT NULL, "is_deleted" int NOT NULL CONSTRAINT "DF_9e2ea14906fc900f6a83b7281c5" DEFAULT 0, "delete_date" datetime, CONSTRAINT "PK_232f6973497284cc8acb7bf5efa" PRIMARY KEY ("tournament_id"))`);
        await queryRunner.query(`CREATE TABLE "match" ("match_id" int NOT NULL IDENTITY(1,1), "match_type" nvarchar(50) NOT NULL CONSTRAINT "DF_7a1ffc439c724032b9b5f0b9f3f" DEFAULT 'scrimmage', "date_held" date NOT NULL, "match_duration" time(3) NOT NULL CONSTRAINT "DF_69b361844d37587ab8c5ba78b0c" DEFAULT '00:00:00.000', "is_deleted" int NOT NULL CONSTRAINT "DF_22a542498dfa76e38b506b73ee9" DEFAULT 0, "delete_date" datetime, "sport_id" int NOT NULL, "tournament_id" int, CONSTRAINT "PK_2e7d516f3dc97d9e2f882212d2b" PRIMARY KEY ("match_id"))`);
        await queryRunner.query(`CREATE TABLE "sport" ("sport_id" int NOT NULL IDENTITY(1,1), "sport_name" nvarchar(75) NOT NULL, "sport_description" nvarchar(250) NOT NULL, "sport_rules" nvarchar(400) NOT NULL, "is_deleted" int NOT NULL CONSTRAINT "DF_e29739e3bbfd72898a5db861a57" DEFAULT 0, "delete_date" datetime, CONSTRAINT "PK_85896d8d20cf619732a5c1fd727" PRIMARY KEY ("sport_id"))`);
        await queryRunner.query(`CREATE TABLE "sponsor" ("sponsor_id" int NOT NULL IDENTITY(1,1), "brand_name" nvarchar(75) NOT NULL, "is_deleted" int NOT NULL CONSTRAINT "DF_e7275a819af5477f6a37e14e1b6" DEFAULT 0, "delete_date" datetime, CONSTRAINT "PK_2c853dd361a7c851f48f2f954b5" PRIMARY KEY ("sponsor_id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "user" ("user_id" int NOT NULL IDENTITY(1,1), "role" nvarchar(15) NOT NULL CONSTRAINT "DF_6620cd026ee2b231beac7cfe578" DEFAULT 'user', "first_name" nvarchar(25) NOT NULL CONSTRAINT "DF_7a4fd2a547828e5efe420e50d1c" DEFAULT 'Guest', "last_name" nvarchar(25) NOT NULL CONSTRAINT "DF_6937e802be2946855a3ad0e6bef" DEFAULT '', "dob" date NOT NULL CONSTRAINT "DF_997a25036ba355bdad2d22cb29b" DEFAULT '', "age" int NOT NULL CONSTRAINT "DF_1f457efff42e9e3d54598c4bd8f" DEFAULT 0, "job" nvarchar(20) NOT NULL CONSTRAINT "DF_a18f709c808e3bde52a89885006" DEFAULT 'guest', "is_deleted" int NOT NULL CONSTRAINT "DF_666851d8509be413462c3b150c0" DEFAULT 0, "delete_date" datetime, CONSTRAINT "PK_758b8ce7c18b9d347461b30228d" PRIMARY KEY ("user_id"))`);
        ...
        await queryRunner.query(`ALTER TABLE "won_team_match" ADD CONSTRAINT "FK_a555138469f01b1405bca488377" FOREIGN KEY ("won_team_id") REFERENCES "team"("team_id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE "lost_team_match" ADD CONSTRAINT "FK_c7cc2ea6232e1c92fcae7ac1d09" FOREIGN KEY ("lost_team_id") REFERENCES "team"("team_id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
